...
def handle_paddle_movement(keys, game: Game):
    if game.ai_agent:
        """
        TODO: Add the conditions when an AI agent has to move and in which direction.
        """
        '''
        if game.left_paddle.move_up is not None:
            if game.left_paddle.move_up and (game.left_paddle.y - game.left_paddle.VEL) >= 0:
                game.left_paddle.move(up=game.left_paddle.move_up)
            elif not game.left_paddle.move_up and (game.left_paddle.y + game.left_paddle.VEL + game.left_paddle.height) <= HEIGHT:
                game.left_paddle.move(up=game.left_paddle.move_up)
        '''
        pass
    else:
...
def main():
    ...
    while run:
        ...
        if not game.won:
            ...
            """
            TODO: Implement the movement of the AI agent
            """            
