...
class AiPaddle(Paddle):
    ...
    def predict(self, ball: Ball):
        ...
        """
        self.pred_y = ball.y - (self.height//2)
        
        if self.pred_y < 0:
            self.pred_y = 0

        if ball.x_vel > 0 and ball.x > WIDTH // 2:
            self.move_up = None
            return

        if self.pred_y - (1.5 * self.VEL) > self.y and self.pred_y + (1.5 * self.VEL) < self.y + self.height:
            self.move_up = None 
        elif self.pred_y < self.y:
            self.move_up = True
        else: 
            self.move_up = False
