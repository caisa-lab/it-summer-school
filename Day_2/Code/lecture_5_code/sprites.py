...
class Spritesheet(object):
    def __init__(self):
        self.sheet = pygame.image.load("spritesheet_white.png").convert()
...
class PacmanSprites(Spritesheet):
    ...
    def update(self, dt, invincibleTimer):
        self.flash = int(invincibleTimer/0.25) % 2 != 0
    ...
    def getImage(self, x, y):
        ss_image = Spritesheet.getImage(self, x, y, 2*TILEWIDTH, 2*TILEHEIGHT).copy()
        colorImage = pygame.Surface(ss_image.get_size()).convert_alpha()
        colorImage.fill(self.color if not self.flash else WHITE)
